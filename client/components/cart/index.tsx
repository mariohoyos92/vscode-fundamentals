...
class Cart extends React.Component<ICartProps, ICartState> {
  ...
  render() {
    ...
    if (cartItems.length === 0) {
      return (
        <ul className="sidedrawer-list">
          <li className="sidedrawer-list__item">Nothing in cart</li>
        </ul>
      );
    }
    ...
    const checkoutConfirmClassName = `checkout-confirm ${
      this.state.checkoutConfirmVisible ? "open" : ""
    }`;

    const checkoutButton = (
      <div className="checkout-btn-container">
        <button
          className="checkout-btn mui-btn mui-btn--accent"
          onClick={() => {
        ...
        <div className={checkoutConfirmClassName}>
          ...
          <button
            ...
            className="mui-btn mui-btn--primary confirm-btn"
          >
            Yes
          </button>
          <button
            onClick={() => this.cancelConfirmCheckout()}
            className="mui-btn mui-btn--secondary cancel-btn"
          >
            No
          </button>
        </div>
